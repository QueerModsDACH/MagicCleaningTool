...
(function () {
    ...
    function isBulkActionActive(action) {
        return Boolean(
            action &&
    ...
    function isBulkActionCancelled(action) {
        if (!action) {
            return false;
        }
        return Boolean(
    ...
    function cancelActiveBulkAction(reason = 'manuell') {
        if (!activeBulkAction) {
            return false;
        }
...
for (const listConfig of activeLists) {
    ...
    function getChannelFromModeratorUrl() {
        const match = window.location.pathname.match( /^\/moderator\/([^/]+)/i );
        if (!match) {
            return null;
        }
        try {
    ...
    function getChannelFromModViewLink() {
        ...
        try {
            const url = new URL( possibleHref, window.location.origin );
            const match = url.pathname.match( /^\/moderator\/([^/]+)/i );
            return match
        ...
        <div id="magicMorningStar" class="magicMorningStar">
            <style>
                ...
                .magicMorningStar .action-bar .streamlabs {
                    ...
                    width: 50px; min-width: 50px; height: 50px; min-height: 50px; padding: 3px; border: 1px solid transparent; border-radius: 50%;
                    background: transparent; color: inherit; box-sizing: border-box; overflow: hidden; appearance: none;
                ...
                .magicMorningStar .action-bar .streamlabs img {
                    display: block; width: 42px; height: 42px; border-radius: 50%; object-fit: cover; transition: transform 180ms ease, filter 180ms ease; }
                ...
                .magicMorningStar .action-bar .streamlabs:not(:disabled):hover {
                    background: rgba(145, 70, 255, 0.18); border-color: rgba(145, 70, 255, 0.65);
                    box-shadow: 0 0 10px rgba(145, 70, 255, 0.35), inset 0 1px 1px rgba(255, 255, 255, 0.3);
                    backdrop-filter: blur(6px); -webkit-backdrop-filter: blur(6px); transform: translateY(-1px) scale(1.03); }
                ...
                .magicMorningStar .action-bar .streamlabs:not(:disabled):hover img {
                    transform: scale(1.04); filter: brightness(1.08) saturate(1.08); }
            ...
            <div class="header">
                ...
                <h5 id="header" class="logo">
                    <a href="https://github.com/QueerModsDACH/MagicCleaningTool" target="_blank" rel="noopener noreferrer"
                        ...
                        Magic Cleaning Tool&nbsp;&nbsp;
                        <img src="${activateImage}" alt="Repository öffnen" width="18" height="18" style="vertical-align: middle;" >
            ...
            <div id="import" class="import" style="display: none;">
                ...
                <div style=" display: flex; align-items: center; gap: 8px; margin-left: 16px; margin-top: 16px; text-align: left; font-size: 12px;" >
                    ...
                    <input type="text" id="banReason" style="width: 66%;" placeholder=" Hier OPTIONAL einen eigenen Bann-Grund angeben" >
                </div>
            </div>
            ...
            <div id="buttons" class="action-bar">
                ...
                <div class="action-group action-group-center">
                        <button class="commanderRoot" type="button" title="Öffnet CommanderRoot" aria-label="CommanderRoot öffnen">
                            <img src="${buttonCommanderRootImage}" alt="" width="42" height="42" aria-hidden="true">
                        </button>
                        <button class="chatstats" type="button" title="Öffnet SullyGnome-Kanalstatistiken" aria-label="SullyGnome-Kanalstatistiken öffnen">
                            <img src="${buttonSullyGnomeImage}" alt="" width="42" height="42" aria-hidden="true">
                        </button>
                        <button class="nightBot" type="button" title="Öffnet NightBot" aria-label="NightBot öffnen">
                            <img src="${buttonNightBotImage}" alt="" width="42" height="42" aria-hidden="true">
                        </button>
                        <button class="fossabot" type="button" title="Öffnet FOSSABOT" aria-label="FOSSABOT öffnen">
                            <img src="${buttonFossabotImage}" alt="" width="42" height="42" aria-hidden="true">
                        </button>
                        <button class="mooBot" type="button" title="Öffnet MooBot" aria-label="MooBot öffnen">
                            <img src="${buttonMooBotImage}" alt="" width="42" height="42" aria-hidden="true">
                        </button>
                        <button class="streamelements" type="button" title="Öffnet StreamElements" aria-label="StreamElements öffnen">
                            <img src="${buttonStreamElementsImage}" alt="" width="42" height="42" aria-hidden="true">
                        </button>
    ...
    function makeToolDraggable() {
        ...
        tool.addEventListener(
            ...
            (event) => {
                ...
                const newLeft = startLeft + (event.clientX - startPointerX);
                const newTop = startTop + (event.clientY - startPointerY);
...
function toggleQuickCheckAuto() {
    ...
    async function checkVersion() {
        const versionElement = d.querySelector('#manupdate');
        if (!versionElement) {
            return;
        }
        try {
    ...
    function setupButtonEvents() {
        ...
        LIST_BUTTONS.forEach((listConfig) => {
            ...
            button.onclick = () => importListByNumber(listConfig.number);
        });
        ...
        d.addEventListener('click', (event) => {
            const target = event.target.closest( 'button, .toggleImport, .start' );
            if (!target) {
                return;
            }
            if (
    ...
    function importList() {
        ...
        lastBulkActionResult = null;
        const importTextarea = d.querySelector('.import textarea');
        if (!importTextarea) {
            return;
        }
    ...
    function importMDGGeneric(listConfig) {
        ...
        const normalizedAction =
            action === 'unban'
                ? 'unban'
                : 'ban';
        const importChannel = activeChannel;
...
if (sourceButton) {
    ...
    async function banAll() {
        ...
        const action = createBulkAction('ban');
        if (!action) {
            return;
        }
        activeBulkAction = action;
...
if (
    ...
    function unbanItem(
        ...
        const actionChannel = action?.channel || activeChannel;
        const normalizedUser = normalizeUser(user);
        if (!isValidUsername(normalizedUser)) {
...
if (
    ...
    function addModChannel(user) {
        const displayUser = String(user ?? '').trim();
    ...
    function updateBulkActionControls() {
        const isRunning = activeBulkAction !== null;
...
const processedCount = completedCount;
    ...
    function renderList() {
        ...
        const hiddenItemCount = Math.max(0, queueList.size - visibleItems.length);
    ...
    function modMenu() {
        ...
        function renderDropdownList(channels = null) {
            ...
            const storedChannels = channels || processStoredModChannels();
